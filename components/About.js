...
const stats = [
  ...
  { label: "Years building AI", value: "1+" },
  { label: "LLM providers", value: "4" },
];

export default function About() {
  return (
    <section id="about" className="relative py-16 md:py-28 border-t border-line">
      <div className="max-w-6xl mx-auto px-5 md:px-8">
        <SectionLabel index="01" title="About" />

        {/* Text + Stats stacked on mobile, side by side on md+ */}
        <div className="flex flex-col md:grid md:grid-cols-5 gap-10 md:gap-12">
          ...
          <motion.div
            ...
            className="md:col-span-2 grid grid-cols-2 gap-3 sm:gap-5"
          >
            {stats.map((s) => (
              <div
                key={s.label}
                className="border border-line rounded-2xl p-4 sm:p-5 bg-surface/50"
              >
